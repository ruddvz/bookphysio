...
/**
 * v2 overlay for the patient messages page.
 * Self-gates via useUiV2() — returns null when v2 is off so the existing
 * v1 page renders byte-identically.
 *
 * This component wraps the full messages layout (conversation sidebar +
 * thread) with v2 chrome: unread Badge chips on conversations, day-grouped
 * thread dividers, and improved empty-state illustration.
 */
...
import type { MessageItem } from './messages-v2-utils'
  ...
  return (
    ...
    >
      <div
        ...
        aria-hidden="true"
      >
        <Inbox className="w-8 h-8 text-[var(--color-pt-primary)]" />
      </div>
  ...
  return (
    ...
    >
      <div
        ...
        aria-hidden="true"
      >
        <MessageSquare className="w-8 h-8 text-[var(--color-pt-primary)]" />
      </div>
      ...
      <p className="text-[13px] text-slate-500 max-w-[240px]">
        Choose a provider from the list to view your message history and start chatting.
      </p>
    </div>
  )
}

export function PatientMessagesV2() {
  const isV2 = useUiV2()
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between shrink-0">
        <div>
          ...
          <p className="text-[13px] text-slate-500 mt-1">
            Communicate directly with your physiotherapy providers
          </p>
        </div>
        {totalUnread > 0 && (
          <Badge
      ...
      <div className="flex-1 min-h-0 flex gap-4 lg:gap-6">
        ...
        >
          ...
          <div className="relative group shrink-0">
            <input
              ...
              className="w-full pl-9 pr-4 py-2 bg-white border border-slate-200 rounded-[var(--sq-sm)] text-[13px] focus:ring-2 focus:ring-[var(--color-pt-primary)] transition-all outline-none"
            />
            <Search
              className="absolute left-2.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-[var(--color-pt-primary)]"
              aria-hidden="true"
            />
          </div>
          ...
          >
            {conversationsLoading ? (
              <div className="flex items-center justify-center py-10">
                <Loader2 className="w-5 h-5 animate-spin text-[var(--color-pt-primary)]" aria-hidden="true" />
              </div>
            ) : conversationsError ? (
              <div className="p-4">
                <button
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isActive && 'bg-[var(--color-pt-tile-1-bg)]'
                    )}
                    data-testid={`v2-conversation-row-${chat.id}`}
                  >
                    {/* Avatar */}
                    <div
                      className="w-10 h-10 rounded-full bg-[var(--color-pt-tile-2-bg)] text-[var(--color-pt-tile-2-fg)] flex items-center justify-center text-[13px] font-bold shrink-0 mt-0.5"
                      aria-hidden="true"
                    >
                      {initials}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-[13px] font-semibold text-slate-900 truncate">
                          {chat.other_user?.full_name ?? 'Provider'}
                        </span>
        ...
        <div
          className={cn(
            'flex-1 min-h-0 flex flex-col bg-white rounded-[var(--sq-lg)] border border-[var(--color-pt-border)] overflow-hidden',
            !selectedConversationId && 'hidden xl:flex'
          )}
        ...
        >
          {activeChat ? (
            <>
              {/* Thread header */}
              <div className="shrink-0 px-5 py-3.5 border-b border-[var(--color-pt-border-soft)] flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div
                    className="w-9 h-9 rounded-full bg-[var(--color-pt-tile-2-bg)] text-[var(--color-pt-tile-2-fg)] flex items-center justify-center text-[12px] font-bold"
                    aria-hidden="true"
                  >
                  ...
                  <div>
                    <p className="text-[14px] font-semibold text-slate-900 leading-tight">
                      {activeChat.other_user?.full_name ?? 'Provider'}
                    </p>
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" aria-hidden="true" />
                      <Badge role="patient" variant="success" data-testid="v2-online-badge">
                        Available
                      </Badge>
                    </div>
                  </div>
              ...
              >
                {messagesLoading ? (
                  <div className="flex items-center justify-center py-10">
                    <Loader2 className="w-6 h-6 animate-spin text-[var(--color-pt-primary)]" aria-hidden="true" />
                  </div>
                ) : messagesError ? (
                  <button
                          ...
                          >
                            <div
                              className={cn(
                                ...
                                isSent
                                  ? 'bg-[var(--color-pt-primary)] text-white rounded-tr-none'
                            ...
                            <div className="flex items-center gap-1.5 mt-1 px-1">
                              ...
                              {isSent &&
                                (msg.read_at ? (
                                  <CheckCheck size={11} className="text-[var(--color-pt-primary)]" aria-label="Read" />
                                ) : (
              ...
              {/* Compose bar */}
              <div className="shrink-0 p-4 border-t border-[var(--color-pt-border-soft)]">
                <div className="flex items-center gap-3">
                  <div className="flex-1 relative">
                    <textarea
                      ...
                      className="w-full pl-4 pr-12 py-2.5 bg-slate-50 border border-slate-200 rounded-[var(--sq-sm)] text-[13px] focus:bg-white focus:ring-2 focus:ring-[var(--color-pt-primary)] outline-none transition-all resize-none"
                    />
                    <button
                      onClick={() =>
                      ...
                      className="absolute right-1.5 top-1.5 w-8 h-8 bg-[var(--color-pt-primary)] text-white rounded-[var(--sq-xs)] flex items-center justify-center hover:opacity-90 disabled:opacity-40 transition-all"
                    >
...
// Re-export the v1 date helper so the v1 page can still use it
export { formatConversationDateDivider }
