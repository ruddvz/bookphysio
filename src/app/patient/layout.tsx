...
import { Home, CalendarDays, MessageSquare } from 'lucide-react'
...
export default function PatientLayout({ children }: { children: ReactNode }) {
  ...
  const displayName = (user?.user_metadata?.full_name as string | undefined) ?? user?.phone ?? 'You'
        ...
        <div className="max-w-[1142px] mx-auto px-6 h-full flex items-center justify-between">
          ...
          <nav className="flex items-center gap-1">
            {navItems.map(({ href, label, icon: Icon }) => {
              ...
              return (
                <Link
                  key={href}
                  href={href}
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg text-[14px] font-medium no-underline transition-colors ${isActive
            ...
            <button
              ...
              disabled={loading}
              title="Sign out"
              className="ml-3 w-9 h-9 rounded-full bg-[#00766C] text-white flex items-center justify-center text-[13px] font-bold cursor-pointer hover:bg-[#005A52] transition-colors"
            >
              {initials}
            </button>
          </nav>
