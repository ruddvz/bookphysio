...
export default function LoginPage({ locale }: { locale?: StaticLocale } = {}) {
  ...
  return (
    <div className="bg-white rounded-3xl p-8 pb-10 sm:p-12 sm:pb-12 max-w-[440px] w-full shadow-2xl shadow-bp-primary/5 border border-bp-border animate-in fade-in slide-in-from-bottom-8 duration-700">
      ...
      <div className="flex bg-bp-surface p-1.5 rounded-2xl mb-10 border border-bp-border shadow-inner">
        <button
          onClick={() => setLoginMode('phone')}
          className={cn(
            "flex-1 py-4 text-[13px] font-bold rounded-xl transition-all duration-300",
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-8">
        {loginMode === 'phone' ? (
          <div>
            <label htmlFor="phone" className="block text-[11px] font-bold text-bp-body/40 uppercase tracking-[0.2em] mb-3 ml-1">{t.loginLabelPhone}</label>
            <div className={cn(
              "flex border-2 rounded-2xl overflow-hidden transition-all duration-500 group bg-bp-surface/50",
              ...
              <span className="px-6 py-5 text-[17px] font-bold text-bp-primary bg-bp-surface border-r-2 border-bp-border flex items-center gap-2 group-focus-within:text-bp-accent transition-colors">
                +91
              </span>
              <input
                id="phone"
                ...
                className="flex-1 px-6 py-5 text-[18px] font-bold text-bp-primary border-none outline-none bg-transparent placeholder:text-bp-body/30 tracking-tight"
              />
            </div>
          ...
          <div>
            <label htmlFor="email" className="block text-[11px] font-bold text-bp-body/40 uppercase tracking-[0.2em] mb-3 ml-1">{t.loginLabelEmail}</label>
            <div className={cn(
              ...
              <input
                ...
                onFocus={() => setInputFocused(true)}
                onBlur={() => setInputFocused(false)}
                className="flex-1 px-6 py-5 text-[18px] font-bold text-bp-primary border-none outline-none bg-transparent placeholder:text-bp-body/30"
              />
            </div>
        ...
        <button
          ...
          disabled={loading}
          className={cn(
            "w-full flex items-center justify-center gap-3 py-5 text-[16px] font-bold text-white rounded-2xl transition-all active:scale-[0.98] relative overflow-hidden group shadow-xl",
