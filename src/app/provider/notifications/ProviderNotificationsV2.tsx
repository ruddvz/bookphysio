...
/** v2 overlay for provider notifications (parity with patient slice 16.19). */
  ...
  return (
    ...
    >
      <div
        ...
        aria-hidden="true"
      >
        <BellOff className="w-8 h-8 text-[var(--color-pv-primary)]" />
      </div>
      <p className="text-[15px] font-semibold text-slate-800 mb-1">All caught up!</p>
      <p className="text-[13px] text-slate-500 max-w-[220px]">
        Your notifications will appear here when there are updates to your practice.
      </p>
    </div>
  )
}

export function ProviderNotificationsV2() {
  const isV2 = useUiV2()
        ...
        <div className="flex items-center gap-3 shrink-0 mt-1">
          ...
          {unreadCount > 0 && (
            <button
              ...
              className="flex items-center gap-1.5 text-[12px] font-semibold text-[var(--color-pv-primary)] hover:opacity-80 disabled:opacity-50 transition-opacity"
            >
        ...
        <div className="space-y-6" data-testid="v2-notification-groups">
          {dayGroups.map(({ dayKey, label, items }) => (
            <div key={dayKey} data-testid={`v2-day-group-${dayKey}`}>
              ...
              {/* Notification rows */}
              <div
                className="bg-white border border-[var(--color-pv-border)] rounded-[var(--sq-lg)] divide-y divide-[var(--color-pv-border-soft)] overflow-hidden"
                role="list"
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        !n.read
                          ? 'bg-[var(--color-pv-tile-1-bg)]/30 hover:bg-[var(--color-pv-tile-1-bg)]/60 cursor-pointer'
                    ...
                    >
                      ...
                      <div
                        className={cn(
                          ...
                          !n.read
                            ? 'bg-[var(--color-pv-tile-1-bg)] text-[var(--color-pv-primary)]'
                      ...
                      <div className="flex flex-col items-end gap-1.5 shrink-0">
                        ...
                        {!n.read && (
                          <span
                            className="w-2 h-2 rounded-full bg-[var(--color-pv-primary)]"
                            aria-label="Unread"
