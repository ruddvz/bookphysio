...
const SLOT_STYLES: Record<SlotStatus, string> = {
  ...
  available: 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100 cursor-pointer',
  blocked: 'bg-slate-50 text-slate-500/60',
  empty: 'bg-white',
}

function SlotCell({ slot }: { slot: Slot }) {
  if (slot.status === 'empty') {
    return <div className="h-14 border border-dashed border-slate-200 rounded-lg" />
  }
  ...
  return (
    <div className="max-w-[1040px] mx-auto px-6 py-12 animate-in fade-in duration-500 delay-100 fill-mode-both">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <h1 className="text-[32px] font-bold text-slate-900 tracking-tight">Calendar</h1>
        <Link
          href="/provider/availability"
          className="inline-flex items-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-full no-underline text-[14px] font-semibold transition-colors self-start sm:self-auto"
        >
      ...
      <div className="bg-white rounded-[12px] border border-slate-200 shadow-sm overflow-hidden">
        {/* Toolbar */}
        <div className="px-6 py-4 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center gap-3 justify-between">
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={prevWeek}
              className="w-9 h-9 flex items-center justify-center border border-slate-200 rounded-lg bg-white hover:bg-[#F9FAFB] transition-colors cursor-pointer outline-none"
            ...
            <button
              type="button"
              onClick={goToday}
              className="px-3 py-1.5 border border-slate-200 rounded-lg text-[13px] font-medium text-slate-900 hover:bg-[#F9FAFB] transition-colors cursor-pointer outline-none"
            >
              Today
            </button>
          </div>
          ...
          <div className="flex items-center gap-4 text-[12px] text-slate-500">
            ...
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-3 rounded-sm bg-emerald-50 inline-block" />
              Available
            </span>
            <span className="flex items-center gap-1.5">
              ...
              <div className="grid grid-cols-[64px_repeat(7,1fr)] border-b border-slate-200">
                ...
                {days.map((d) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`text-[11px] font-medium uppercase tracking-wide ${isToday ? 'text-emerald-600' : 'text-slate-500/60'}`}>
                        {formatDay(d)}
                      </div>
                ...
                {HOURS.map((h) => (
                  <div key={h} className="grid grid-cols-[64px_repeat(7,1fr)] items-start gap-x-1 px-1 py-1">
                    <div className="flex items-start justify-end pr-3 pt-1">
                      <span className="text-[11px] text-slate-500/60 font-medium">{formatHour(h)}</span>
                    </div>
                    {days.map((d) => {
                      const key = dateKey(d)
