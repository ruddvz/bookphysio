...
export const providerProfileSchema = z.object({
  ...
  certifications: z.array(z.string().trim().min(1).max(100)).max(20).optional(),
  equipment_tags: z.array(z.string().trim().min(1).max(100)).max(30).optional(),
})

/** Schema for the private IAP member ID — used only in admin/owner-restricted routes. */
export const providerIapSchema = z.object({
  iap_member_id: z.string().trim().min(1).max(50),
})
