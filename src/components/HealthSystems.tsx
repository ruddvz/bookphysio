...
function FeatureCard({ icon: Icon, title, description }: (typeof features)[number]) {
  return (
    <div className="bp-card-soft p-5 transition-all duration-300 hover:-translate-y-1 hover:border-[#0f7668]/20 hover:bg-white">
      <div className="flex h-12 w-12 items-center justify-center rounded-[18px] bg-[#dcefe9] text-[#0f7668]">
        <Icon size={20} />
      </div>
      <h3 className="mt-4 text-[20px] font-semibold tracking-[-0.03em] text-[#18312d]">{title}</h3>
          ...
          <div className="max-w-xl">
            ...
            <div className="mt-8 space-y-3">
              {['ICP verified providers', 'In-clinic + home visit modes', 'Transparent INR pricing', 'Mobile-first booking flow'].map((item) => (
                <div key={item} className="flex items-center gap-3 rounded-[20px] border border-[#ddd3c6] bg-[#fffdf9] px-4 py-3 text-[14px] text-[#5d6662]">
                  <ShieldCheck size={16} className="text-[#0f7668]" />
                  {item}
                </div>
              ))}
            </div>
          ...
          <div className="space-y-5">
            ...
            <div className="bp-card flex flex-col gap-4 p-5 md:flex-row md:items-center md:justify-between">
              <div className="flex items-center gap-3">
                <div className="flex h-12 w-12 items-center justify-center rounded-[18px] bg-[#18312d] text-white">
                  <MapPin size={20} />
                </div>
                <div>
                  <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#7d8a85]">Coverage</p>
                  <p className="text-[18px] font-semibold tracking-[-0.03em] text-[#18312d]">18 Indian cities and growing</p>
                </div>
              </div>
