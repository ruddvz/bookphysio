...
import { getVariantCopy } from '@/components/book-physio-ai-chat/variant-copy'
import type { AIChatVariant, Tone } from '@/components/book-physio-ai-chat/types'
  ...
  return content.split(/(\[[CK]\d+\])/g).map((part, index) => {
    if (/\[[CK]\d+\]/.test(part)) {
      return (
        <span
          key={`${part}-${index}`}
          className="mx-1 inline-flex items-center rounded-full border border-bp-accent/20 bg-bp-accent/10 px-2.5 py-0.5 text-[11px] font-bold text-bp-accent"
        >
          {part}
        </span>
      )
          ...
          <aside className="hidden xl:flex flex-col gap-6">
            <div className="rounded-3xl border border-bp-border bg-white/90 p-6 shadow-[0_28px_80px_-48px_rgba(0,0,0,0.25)] backdrop-blur-xl">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-bp-body/40">{copy.eyebrow}</p>
                  <h1 className="text-[20px] font-bold tracking-tight text-bp-primary">Ace</h1>
                </div>
              </div>

              <div className="mt-5 rounded-[28px] border border-bp-border bg-[#fafbfc] p-5">
                <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-bp-accent">One AI for the platform</p>
                  ...
                  {copy.metrics.map((metric) => {
                    ...
                    return (
                      <div key={metric.label} className="rounded-2xl border border-bp-border bg-white p-3 shadow-sm">
                        ...
                        <p className="mt-1 text-[16px] font-bold tracking-tight text-bp-primary">{metric.value}</p>
                      </div>
                    )
                  })}
                </div>
              ...
              <div className="grid gap-6 p-6 md:p-8 lg:grid-cols-[1.25fr_0.75fr] lg:items-end">
                <div className="space-y-5">
                  ...
                  <div className="space-y-3">
                    <p className="text-[11px] font-bold uppercase tracking-[0.24em] text-bp-accent">{copy.eyebrow}</p>
                    <h1 className="max-w-3xl text-[34px] font-bold leading-[0.95] tracking-tight text-bp-primary md:text-[52px]">BookPhysio AI</h1>
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <Link
                      ...
                      className="inline-flex items-center gap-3 rounded-[24px] bg-bp-primary px-6 py-3.5 text-[14px] font-bold text-white shadow-xl shadow-gray-200 transition-all hover:-translate-y-0.5 hover:bg-bp-accent"
                    >
                      {copy.primaryLabel}
                    ...
                    <Link
                      ...
                      className="inline-flex items-center gap-3 rounded-[24px] border border-bp-border bg-white px-6 py-3.5 text-[14px] font-bold text-bp-primary shadow-sm transition-all hover:border-bp-accent/20 hover:text-bp-accent"
                    >
                      {copy.secondaryLabel}
                  ...
                  {copy.metrics.map((metric) => {
                    ...
                    return (
                      <div key={metric.label} className="rounded-2xl border border-bp-border bg-[#fafbfc] p-4 shadow-sm">
                        ...
                        <p className="mt-4 text-[11px] font-bold uppercase tracking-widest text-bp-body/40">{metric.label}</p>
                        <p className="mt-1 text-[18px] font-bold tracking-tight text-bp-primary">{metric.value}</p>
              ...
              <section className="flex min-h-[760px] flex-col overflow-hidden rounded-3xl border border-bp-border bg-white shadow-[0_28px_80px_-42px_rgba(0,0,0,0.22)]">
                <div className="flex items-center justify-between border-b border-bp-border px-6 py-5 md:px-8">
                  <div className="space-y-1">
                    ...
                    <h2 className="text-[20px] font-bold tracking-tight text-bp-primary">Conversation canvas</h2>
                  </div>
                  <div className="inline-flex items-center gap-2 rounded-full border border-bp-accent/20 bg-bp-accent/10 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.22em] text-bp-accent">
                          ...
                          {citationIds.length > 0 && !isUser && (
                            <div className="mt-4 flex flex-wrap gap-2 border-t border-bp-border/50 pt-4">
                              {citationIds.map((citationId) => (
                                <span
                                  key={citationId}
                                  className="inline-flex items-center gap-2 rounded-full border border-bp-accent/20 bg-bp-accent/10 px-3 py-1 text-[11px] font-bold text-bp-accent"
                                >
                ...
                <div className="border-t border-bp-border bg-[#fafbfc] px-6 py-5 md:px-8 md:py-6">
                  <div className="mb-4 flex flex-wrap gap-2">
                    {copy.quickPrompts.map((prompt) => (
                      <button
                        ...
                        className="inline-flex items-center gap-2 rounded-full border border-bp-border bg-white px-4 py-2 text-[12px] font-bold text-bp-primary shadow-sm transition-all hover:-translate-y-0.5 hover:border-bp-accent/20 hover:text-bp-accent"
                      >
                  ...
                  <p className="mt-4 text-center text-[10px] font-bold uppercase tracking-[0.22em] text-bp-body/30">{copy.footerNote}</p>
                </div>
              </section>
                        ...
                        <>
                          <div className="flex items-start gap-3">
                            ...
                            <div className="min-w-0 flex-1">
                              <p className={cn('inline-flex rounded-full border px-2.5 py-1 text-[9px] font-bold uppercase tracking-[0.2em]', styles.badge)}>
                                {card.label}
                              </p>
                              <h3 className="mt-3 text-[15px] font-bold tracking-tight text-bp-primary">{card.title}</h3>
