...
Slice **16.22** is complete: `/patient/search` — in-dashboard provider search with v2 `PatientSearchFiltersRail` when ui-v2 (specialty, 6-digit pincode, visit mode, availability sort). Optional `pincode` on `GET /api/providers` filters via relational fallback.
